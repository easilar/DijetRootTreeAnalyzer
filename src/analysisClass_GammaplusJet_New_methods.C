...
bool isNewonOldValidJetTight(const float& Eta_ak4, const float& nhf, const float& neMult, const float& chMult, const float& nemf, const float& muF, const float& chf, const float& cemF, const float& JecCorr)
{
  bool idL = false ;
  if(fabs(Eta_ak4) > 3.0){
    idL = ( neMult > 10 && nhf*JecCorr > 0.2 && nemf*JecCorr < 0.9 );
  }
  if(fabs(Eta_ak4) > 2.7  && fabs(Eta_ak4) <= 3.0){
    idL = ( nemf*JecCorr < 0.99 && nemf*JecCorr>0.01 && neMult>1)  ;
  }
    if(fabs(Eta_ak4) >= 2.6  && fabs(Eta_ak4) <= 2.7){
      idL = (nhf*JecCorr < 0.9 && nemf*JecCorr < 0.99  && muF*JecCorr < 0.8 && chMult>0 && cemF*JecCorr<0.8);
    }
    if( fabs(Eta_ak4) < 2.6 ){
  ...
  :baseClass(inputList, cutFile, treeName, outputFileName, cutEfficFile)
{
  ...
  if( int(getPreCutValue1("useJECs"))==1 )
  {
    ...
    //uncertainty
    std::cout<<" JecuncData "<<getPreCutString1("JECuncDat")<<endl;
    std::cout<<" JecuncMC "<<getPreCutString1("JECuncMC")<<endl;
   ...
   //for (Long64_t jentry=0; jentry<2000;jentry++) {
    ...
    if(jentry < 10 || jentry%100000 == 0) std::cout << "analysisClass::Loop(): jentry = " << jentry << std::endl;   
     ...
     ///Stuff to be done for every event
      //Checking the BunchCrossing,to define if running on data or MC...
      //SB Change to direct reading of new leaf from the TreeMaker
     int idx_InTimeBX=-1;
	...
	if(!HaspixelSeed->at(indexgoodpho)){
	  if(PhotonLoosePt->at(indexgoodpho)>= 40.){
		...
		Electron.SetPtEtaPhiE(electronPt->at(j),electronEta->at(j),electronPhi->at(j),electronEnergy->at(j));
		double deltaR = gamma1.DeltaR(Electron);
		if (deltaR < 0.13) {
		  keepEvent = false;
		  break;
		}
		...
		for(std::multimap<double, unsigned>::const_reverse_iterator it = sortedJets.rbegin(); it != sortedJets.rend(); ++it)sortedJetIdx.push_back(it->second);
	      }else{
		if(int(getPreCutValue1("noJECs"))==1  ){
		  // sort jets by increasing pT
		  std::multimap<double, unsigned> sortedJets;
		  for(size_t j=0; j<no_jets_ak4; ++j){ //same ordering of original root trees
		    ...
		    //SB Add Photon removal as for the case of JEC
		    if( gamma1.DeltaR(Tmpjet) < 0.4){ 
		      nfakejet++ ;
		      fakejetIdx = j;
		      fakecorrection = 1.;
		...
		//SB, add a stupid,impossible cout on hasgen to avoid compilation error of variable set but not used (and I do not want to change largely the code)
		if(!isData && jetPtGenAK4->at(sortedJetIdx[0])>=0.){ hasgen = 1;if(!hasgen)cout<<hasgen<<endl;}
	      //SB: Need to implement a loop to choose the leading jet in case it does not pass the jet ID:
	      for(int leadjet=0;leadjet<int(no_jets_ak4 - nfakejet);leadjet++){
		if(!isData && jetPtGenAK4->at(sortedJetIdx[leadjet])>=0.){ hasgen = 1;}
		//Hardcoded: Leading jet selected pt>15 GeV
		if((CorrFactors[sortedJetIdx[leadjet]]/jetJecAK4->at(sortedJetIdx[leadjet]))*jetPtAK4->at(sortedJetIdx[leadjet]) >=  15.  && isNewonOldValidJetTight(jetEtaAK4->at(sortedJetIdx[leadjet]), jetNhfAK4->at(sortedJetIdx[leadjet]), neMultAK4->at(sortedJetIdx[leadjet]), chMultAK4->at(sortedJetIdx[leadjet]), jetNemfAK4->at(sortedJetIdx[leadjet]), jetMufAK4->at(sortedJetIdx[leadjet]), jetChfAK4->at(sortedJetIdx[leadjet]), jetCemfAK4->at(sortedJetIdx[leadjet]), jetJecAK4->at(sortedJetIdx[leadjet]))){
		  ak4j1.SetPtEtaPhiM( (CorrFactors[sortedJetIdx[leadjet]]/jetJecAK4->at(sortedJetIdx[leadjet])) *jetPtAK4->at(sortedJetIdx[leadjet]) ,jetEtaAK4->at(sortedJetIdx[leadjet])
				      ,jetPhiAK4->at(sortedJetIdx[leadjet])
				      , (CorrFactors[sortedJetIdx[leadjet]]/jetJecAK4->at(sortedJetIdx[leadjet])) *jetMassAK4->at(sortedJetIdx[leadjet]));
		  
		  for(size_t secjet = leadjet ; secjet < (no_jets_ak4 - nfakejet) ; secjet++ ){	
		    if(!isData && jetPtGenAK4->at(sortedJetIdx[secjet] > 0.)){ hasgen2 = 1;}	     
		    if(no_jets_ak4-nfakejet >= secjet + 1 && (CorrFactors[sortedJetIdx[secjet]]/jetJecAK4->at(sortedJetIdx[secjet]))*jetPtAK4->at(sortedJetIdx[secjet]) >= 10 && isNewonOldValidJetTight(jetEtaAK4->at(sortedJetIdx[secjet]), jetNhfAK4->at(sortedJetIdx[secjet]), neMultAK4->at(sortedJetIdx[secjet]), chMultAK4->at(sortedJetIdx[secjet]), jetNemfAK4->at(sortedJetIdx[secjet]), jetMufAK4->at(sortedJetIdx[secjet]), jetChfAK4->at(sortedJetIdx[secjet]), jetCemfAK4->at(sortedJetIdx[secjet]),jetJecAK4->at(sortedJetIdx[secjet]))){
		      ak4j2.SetPtEtaPhiM( (CorrFactors[sortedJetIdx[secjet]]/jetJecAK4->at(sortedJetIdx[secjet])) *jetPtAK4->at(sortedJetIdx[secjet])
					  ,jetEtaAK4->at(sortedJetIdx[secjet])
					  ,jetPhiAK4->at(sortedJetIdx[secjet])
					  , (CorrFactors[sortedJetIdx[secjet]]/jetJecAK4->at(sortedJetIdx[secjet])) *jetMassAK4->at(sortedJetIdx[secjet]));
		      sndjetidx = secjet;
		      break;//end of loop on secjet
		    }//end if jet ID
		    ak4j2.SetPtEtaPhiM(0.,0.,0.,0.);
		  }//end loop secjet
		  leadjetidx=leadjet;
		  break;//end of loop on leadjet
		}//SB: Loop added on leading jet, so we always find a leading jet if more than one jet
		// 		  else{//Leading jet is not passing jet ID
		// 		    ncut_ptjet++;
		// 		    continue;
		// 		  }
	      }
	      }else{//No jets in the event
		ncut_jet++;
		continue;}
		...
		for(int i = 0 ; i < int(no_jets_ak4 - nfakejet); i++){
		  ...
		  //SB: Here again a hardcoded pt cut at 2GeV to store all jets
		  if((CorrFactors[sortedJetIdx[i]]/jetJecAK4->at(sortedJetIdx[i]))*jetPtAK4->at(sortedJetIdx[i]) >=  2. ){
		    ...
		    bool isID = false;
		    if(isNewonOldValidJetTight(jetEtaAK4->at(sortedJetIdx[i]), jetNhfAK4->at(sortedJetIdx[i]), neMultAK4->at(sortedJetIdx[i]), chMultAK4->at(sortedJetIdx[i]), jetNemfAK4->at(sortedJetIdx[i]), jetMufAK4->at(sortedJetIdx[i]), jetChfAK4->at(sortedJetIdx[i]), jetCemfAK4->at(sortedJetIdx[i]),jetJecAK4->at(sortedJetIdx[i]))) isID = true; 
	      ...
	      //SB: Retrieve the value of the correction for each step of the correction applied
	      if(nPhotonTight == 1){
		for (int it=0; it< int(no_jets_ak4-nfakejet) ; it++) {
		  if(fakejetIdx==it)continue; //SB, fake jet is the photon-jet		  
		  double corrs = 1.;
		  ...
		  if(isData){
		    ...
		    JetCorrectortypI->setJetPt(jetPtAK4->at(it)/jetJecAK4->at(it)); //SB: saving Raw info
		  ...
		  //SB: Remove this STRANGE cut about not using endcap jets
		  ...
		  if(corrJet.Pt() > 15. && dR > 0.25) {
		    ...
		    //SB the cut does not really make sense as the photon should be already removed, commented it out
		...
		//== Fill Variables ==
		bool hasgen = false;
		if(!isData && jetPtGenAK4->at(sortedJetIdx[leadjetidx])){ hasgen = 1;}
		if(ak4j1.Pt()>0 && isNewonOldValidJetTight(jetEtaAK4->at(sortedJetIdx[leadjetidx]), jetNhfAK4->at(sortedJetIdx[leadjetidx]), neMultAK4->at(sortedJetIdx[leadjetidx]), chMultAK4->at(sortedJetIdx[leadjetidx]), jetNemfAK4->at(sortedJetIdx[leadjetidx]), jetMufAK4->at(sortedJetIdx[leadjetidx]), jetChfAK4->at(sortedJetIdx[leadjetidx]), jetCemfAK4->at(sortedJetIdx[leadjetidx]),jetJecAK4->at(sortedJetIdx[leadjetidx]))){
		    nselectedevent++;
		    if(lumi != storelumi){
		    ...
		    if( AK4jets.size() >=1 ){
		      fillVariableWithValue( "IdTight_j1",idLAK4->at(sortedJetIdx[leadjetidx]));
		      fillVariableWithValue( "pTAK4_j1", AK4jets[leadjetidx].Pt());
		      fillVariableWithValue( "etaAK4_j1", AK4jets[leadjetidx].Eta());
		      fillVariableWithValue( "phiAK4_j1", AK4jets[leadjetidx].Phi());
		      
		      fillVariableWithValue( "jetJecAK4_j1", jecFactors[sortedJetIdx[leadjetidx]] );
		      fillVariableWithValue( "jetJecUncAK4_j1", jecUncertainty[sortedJetIdx[leadjetidx]] );
		      fillVariableWithValue( "jetCSVAK4_j1", jetCSVAK4->at(sortedJetIdx[leadjetidx]) );
		      fillVariableWithValue( "jetQGDAK4_j1", jetQGDAK4->at(sortedJetIdx[leadjetidx]) );
		      //jetID
		      fillVariableWithValue( "neutrHadEnFrac_j1", jetNhfAK4->at(sortedJetIdx[leadjetidx]));
		      fillVariableWithValue( "chargedHadEnFrac_j1", jetChfAK4->at(sortedJetIdx[leadjetidx]));
		      fillVariableWithValue( "photonEnFrac_j1", jetPhfAK4->at(sortedJetIdx[leadjetidx]));
		      fillVariableWithValue( "eleEnFract_j1", jetElfAK4->at(sortedJetIdx[leadjetidx]));
		      fillVariableWithValue( "muEnFract_j1", jetMufAK4->at(sortedJetIdx[leadjetidx]));
		      fillVariableWithValue( "neutrElectromFrac_j1", jetNemfAK4->at(sortedJetIdx[leadjetidx]));
		      fillVariableWithValue( "chargedElectromFrac_j1", jetCemfAK4->at(sortedJetIdx[leadjetidx]));
			...
			fillVariableWithValue( "jetJerAK4_j1", jerFactors[sortedJetIdx[leadjetidx]] );
		      ...
		      if(!isData && hasgen ){
			fillVariableWithValue( "pTAK4_j1GEN", jetPtGenAK4->at(sortedJetIdx[leadjetidx]));
			fillVariableWithValue( "etaAK4_j1GEN", jetEtaGenAK4->at(sortedJetIdx[leadjetidx]));	       
			fillVariableWithValue("PDGIDAK4_j1",jetpdgIDGenAK4->at(sortedJetIdx[leadjetidx]));
		      }
		    }
